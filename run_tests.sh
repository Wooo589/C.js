...
for file in tests/*.txt; do
    ...
    echo -e "${CYAN}Testando:${NC} $base_name"

    expected_err_file="${TEST_DIR}/${base_name}.expected.err"
    expected_out_file="${TEST_DIR}/${base_name}.expected.out"
    ...
    if [ -f "$expected_err_file" ]; then
        ...
        echo -e "  └─ ${YELLOW}[WARN]${NC} Nenhum arquivo .expected.* encontrado para $base_name. Pulando verificação."
    fi
